...
// Extra Step: findContact function
...
function addContact(name, phone, email) {
  if (findContact(name)) {
    console.log(`Warning: a contact named ${name} already exists.`);
    return;
  }
...
function searchByPartialName(partialName) {
  const results = contacts.filter( contact => contact.name.toLowerCase().includes(partialName.toLowerCase()));
  ...
  console.log(`No contacts found containing name: ${partialName}`);
}
  I found the errors here and corected. So the function must be like as following:
  */
