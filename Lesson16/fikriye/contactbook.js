...
const contacts = [
  ...
  {name: 'Fikriye', phone:'123-456-7890', email: 'fikriye@example.com'},
];
...
addContact('name', 'saasdas', 'asdsadsa','jkrhfeurıfn',);
...
*/
function removeContact(name){
  const index = contacts.findIndex(contact => contact.name.toLocaleLowerCase() === name.toLocaleLowerCase());
  if (index !== -1){
    contacts.splice (index,1);
    console.log ("${name} başarıyla silindi.");
  }
  else {console.log("${name} isimli kişi bulunamadı.");
    }
}
...
*/
function searchByNamePartial(partialName, contacts) {
  return contacts.filter(contact =>
    contact.name.toLowerCase().includes(partialName.toLowerCase())
  );
  if (matches.length > 0) {
    console.log(`"${partialName}" için eşleşen kişiler:`);
    matches.forEach(c =>
      console.log(`- ${c.name} (${c.phone}, ${c.email})`)
    );
  } else {
    console.log(`"${partialName}" ile eşleşen kişi bulunamadı.`);
  }
}
function sortContactsByName(contacts) {
  ...
  console.log("Alfabetik sıralı kişi listesi:");
  sorted.forEach(c => console.log(`- ${c.name}`));
}
function findByPhoneOrEmail(query, contacts) {
  const found = contacts.find(contact =>
    contact.phone === query || contact.email === query
  );
  if (found) {
...
findByPhoneOrEmail("fikriye@example.com",contacts);
sortContactsByName(contacts);
searchByNamePartial("fik", contacts);