...
function practiceFlashcard(index) {
  if (index >= 0 && index < flashcards.length) {
    ...
    console.log("Question: ", flashcard.question, "with index:", index);
    setTimeout(() => console.log("The answer is:", flashcard.answer), 3000);
  } else {
    console.log(
      `Flashcard with index ${index} is not in the list. Use index from 0 to ${
        flashcards.length - 1
      }`
    );
  }
...
/*console.log("\nUpdating flashcards...");
updateFlashcard('What is JS?', 'What is JavaScript?', 'A programming language for web development');
updateFlashcard('What is CSS?', 'What is Cascading Style Sheets?', 'Styles the web page');
updateFlashcard('What is HTML?,  "What is Hyper Text Markup Language?", "A language for making web pages"');
...
function removeFlashcard(question) {
  ...
  if (index !== -1) {
    ...
    console.log(`Flashcard with question \'${question}\' removed`);
  } else {
    console.log(`Flashcard with question \'${question}\' not found`);
  }
}
...
removeFlashcard('What is Python?');
...
updateFlashcard(
  ...
  "A language for making web pages"
);
...
console.log("\nRemoving flashcards...");
removeFlashcard("What is HTML?");
removeFlashcard("What is Hyper Text Markup Language?");
...
function showFlashcardsByTag(tag) {
  ...
  for (const flashcard of flashcards1) {
    if (flashcard.tag === tag) {
      console.log(`${flashcard.question} --- ${flashcard.answer}`);
    }
  }
}

showFlashcardsByTag("HTML");
