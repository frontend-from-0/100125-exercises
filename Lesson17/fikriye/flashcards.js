...
*/
function updateFlashcard(questions,newQuestion, newAnswer) {
  for (let i=0; i< flashcards.length; i++) {
  if (flashcards[i].question === questions) {
    flashcards[i].question = newQuestion;
    flashcards[i].answer = newAnswer;
    console.log(`Flashcard updated: ${newQuestion} - ${newAnswer}`);
    return;
  }
  console.log(`Flashcard with question \'${questions}\' not found`);
...
console.log(`Flashcard with question '${question}' was not found`);
}
/*
-----------------------------------------------------------
  STEP 7: Test the App
...
function showAllFlashcards() {
  flashcards.forEach((card, index) => {
    console.log(`${index + 1}. Q: ${card.question} | A: ${card.answer}`);
  });
}
