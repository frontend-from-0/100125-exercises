...
function updateFlashcard(question, newQuestion, newAnswer) {
  for (const card of flashcards) {
    if (card.question === question) {
      console.log(`Question: ', ${question}  is found.`);

      card.question === newQuestion;
      card.answer === newAnswer;
      console.log(
        `New question '${newQuestion}' and new answer '${newAnswer}' are updated.`
      );
      return;
    } 
...
updateFlashcard("What is HTML?", "Why is HTML important for web pages?", "Because it is the language for making web pages.");
...
function removeFlashcard(question) {
  console.log("Removing the found flashcards...");

   const flashcard = {question, answer};

  for (const card of flashcards) {
    if (card.question === question) {
      const foundQuestion = flashcards.indexOf(card);
      flashcards.splice(foundQuestion, 1);
      console.log(`Flashcard with question \'${question}\' has been removed.`);
      return;
    }
  }
  console.log(`Question: ', ${question}  is not found.`);
...
removeFlashcard("Why is HTML important for web pages?");
...
function getRandomFlashcard() {
  ...
  console.log(`Flashcard with question \'${question}\' has been selected randomly.`);
}
getRandomFlashcard();