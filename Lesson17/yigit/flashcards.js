...
function updateFlashcard(question, newQuestion, newAnswer) {
    for (const card of flashcards) {
      if (card.question === question) {
        ...
        console.log("The flashcard has been updated.")
        return;
      }
    }
    console.log("No such flashcard was found.")
}

updateFlashcard('What is HTML?', 'What do we use HTML for?' , 'We use HTML to give our website a basic structure.');
...
function removeFlashcard(question) {
  ...
  if (index !== -1) {
    ...
    console.log(`Flashcard with question '${question}' was removed successfully.`);
  } else {
    console.log(`No flashcard found with the question: '${question}'`);
  }
}

removeFlashcard('What do we use HTML for?');
...
// New flashcard
addFlashcard('What is Python?', 'A programming language used for many purposes');
showAllFlashcards();

// Practice with valid index
practiceFlashcard(1);

// Practice with invalid index
practiceFlashcard(10);

// Updating existing flashcard
updateFlashcard('What is JS?', 'What does JS stand for?', 'JavaScript');

// Updating a non-existent flashcard
updateFlashcard('Nonexistent?', 'New Q', 'New A');

// Remove a flashcard
removeFlashcard('What does CSS do?');

//Removing non-existent flashcard
removeFlashcard('Nonexistent?');
