...
class ShoppingCart {
  ...
  removeItem(name) {
    ...
    console.log(`Item named '${name}' does not exist...`);
  ...
  getTotal() {
    let totalCost = 0;
    for (const item of this._items) {
      const onePiece = item.price.amount * item.quantity;
      totalCost += onePiece;
      console.log(`Total cost is ${totalCost} ${DataTransferItemList.currency}`);
      return;
    }
  }
  ...
  applyDiscount(code) {
    ...
    if (this._discount > 0) {
      const discountAmount = (this._discount / 100) * totalCost;
      totalCost -= discountAmount;
      console.log(`Discount applied: -${discountAmount} (${this._discount}%)`);
    }

    const currency = this._items[0]?.price.currency || 'USD';
    console.log(`Total cost is ${totalCost.toFixed(2)} ${currency}`);
    return totalCost;
  }
  }
