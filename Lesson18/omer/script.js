...
class ShoppingCart {
  ...
  removeItem(name) {
    ...
    if (this._items.length === initialLength) {
      console.log(`${name} isimli urun sepette bulunamadi.`);
    } else {
      console.log(`${name} isimli urun sepetten kaldirildi.`);
    }
  }
  ...
  applyDiscount(code) {
    const discountKey = code.toLowerCase();
    const discount = discounts[discountKey];

    if (!discount) {
      console.log('Gecersiz indirim kodu!');
      return this.getTotal();
    }

const total = this.getTotal();
const discountedTotal = total - (total * discount) / 100;
console.log(`${discount}% indirim uygulandi. Yeni toplam: ${discountedTotal}`);
