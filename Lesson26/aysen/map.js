...
const uppercasedWords = wordsExercise1.map((word) => word.toUpperCase());

console.log(wordsExercise1, uppercasedWords);
...
const namesAndGrades = studentsExercise5.map(
...
const stringRep = numbersExercise6.map((number) => number + '');
...
const sentences = sentencesExercise7.map((sentence) => sentence.split(' '));
...
function getEmailTemplate(userName, userEmail) {
  return `To: ${userEmail}
	Subject: Very imporant news!
Body: Hi ${userName}, here are very important news for you!`;
}

const emails = usersExercise8.map((user) => user.email);
console.log(emails);

const sendEmail = usersExercise8.map((user) =>
  getEmailTemplate(user.name, user.email),
);
...
const books = booksExercise9.map((book)=> `Book Title: ${book.title}  Author:${book.author}`);
console.log(books);
...
const prices = pricesExercise10.map((price) => `$${price}`)
console.log(prices);